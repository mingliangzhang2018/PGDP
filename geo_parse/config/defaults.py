...
_C.MODEL.FCOS.INFERENCE_TH = 0.2
...
# ---------------------------------------------------------------------------- #
_C.MODEL.SEG = CN()
_C.MODEL.SEG.NUM_CLASSES = 4  # point,line,circle and backbone
_C.MODEL.SEG.FPN_STRIDES = 4
# the number of convolutions used in the seg and seg_emb
_C.MODEL.SEG.NUM_CONVS = 4
# the number of embedding dim 
_C.MODEL.SEG.EMB_DIMS = 4
# Discriminative loss parameter: delta_v
_C.MODEL.SEG.LOSS_DELTA_V = 0.5
# Discriminative loss parameter: delta_d
_C.MODEL.SEG.LOSS_DELTA_D = 3.0
# ratio of loss term: var
_C.MODEL.SEG.LOSS_RATIO_VAR = 1.0
# ratio of loss term: dist
_C.MODEL.SEG.LOSS_RATIO_DIST = 1.0
# ratio of loss term: reg
_C.MODEL.SEG.LOSS_RATIO_REG = 0.001
# ratio of loss term: binary_seg
_C.MODEL.SEG.LOSS_RATIO_BS = 1.0
# ratio of geo pixel: weight_geo 
_C.MODEL.SEG.CLASS_WEIGHTS = [10, 3, 10]
# min area of geo
_C.MODEL.SEG.MIN_AREA_RATIO = [0.3, 0.1, 0.1]
# threshold of binary seg
_C.MODEL.SEG.BIN_SEG_TH = [0.5, 0.5, 0.5]
# # training with location map 
# _C.MODEL.SEG.WITH_LOC = False
...
# ---------------------------------------------------------------------------- #
_C.MODEL.VISEMB = CN()
# the number of convolutions used in the visual_emb
_C.MODEL.VISEMB.NUM_CONVS = 4
# the number of embedding dim 
_C.MODEL.VISEMB.EMB_DIMS = 64
# Just for ROIAlign scale
_C.MODEL.VISEMB.FPN_STRIDES = 4
...
_C.MODEL.REL.ROIALIGN_SIZE = [5, 5]
# Node embedding dim
_C.MODEL.REL.NODE_DIMS = 64
# Edge embedding dim
_C.MODEL.REL.EDGE_DIMS = 64
# Node class num
_C.MODEL.REL.NODE_NUM_CLASSES = 6
# Edge class num
_C.MODEL.REL.EDGE_NUM_CLASSES = 2
# Graph layer num
_C.MODEL.REL.NUM_LAYERS= 4
# Graph head num
_C.MODEL.REL.NUM_HEADS = 3
# LeakyReLU alpha
_C.MODEL.REL.RELU_ALPHA = 0.1
# Dropout ratio
_C.MODEL.REL.DROPOUT_RATIO = 0.0
# ratio of loss term: edge classify
_C.MODEL.REL.LOSS_RATIO_EDGE = 4.0
# ratio of loss term: text node classify
_C.MODEL.REL.LOSS_RATIO_NODE = 4.0
# semantic class num
_C.MODEL.REL.SEM_NUM_CLASSES = 10
# Edge classify threshold
_C.MODEL.REL.EDGE_TH = 0.5
_C.MODEL.REL.FEAT_WITH_LOC = True
_C.MODEL.REL.FEAT_WITH_SEM = True
_C.MODEL.REL.GEO2GEO_RULE = False
...
# This is global, so if we have 8 GPUs and IMS_PER_BATCH = 16, each GPU will
# see 2 images per batch
_C.TEST.IMS_PER_BATCH = 9
# Number of detections per image
_C.TEST.DETECTIONS_PER_IMG = 100
# Score threshold
_C.TEST.SCORE_THRESH = 0.5
# IOU threshold of detection
_C.TEST.IOU_DET_TH = 0.5
# IOU threshold of segmentation 
_C.TEST.IOU_SEG_TH = 0.75
